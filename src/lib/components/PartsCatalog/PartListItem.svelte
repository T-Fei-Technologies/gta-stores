<script lang="ts">
  ...
  export let onAddItem: (partId: string, type: string, subtype?: string, quantity?: number) => void;
  ...
  const onItemAdd = () => {
    const newQuantity = ($items.find(itemsPart => itemsPart.partId === part.id)?.quantity ?? 0) + quantity;
    onAddItem(part.id, type, subtype, newQuantity);
